...
場次另外具有可編輯的顯示名稱；場次碼、日期與既有票券識別維持不變。當活動旗下所有票券都已從 `ACTIVE` 完成核銷或撤銷後，後台可永久刪除整個活動。刪除會移除場次、票券、抽取碼、驗證碼、發行重試內容與一般稽核資料，只把 `used_at` 已成立的票券轉成後台可查詢／匯出的核銷快照；活動代碼僅保留為不可見墓碑，避免舊代碼被誤重建。
