...
function normalize(entry){
  ...
  const value={
    ...
    eventName:session.eventName||entry.eventName||'',eventCode:session.eventCode||entry.eventCode||'',sessionCode:session.sessionCode||entry.sessionCode||'',sessionName:session.sessionName||entry.sessionName||session.sessionCode||entry.sessionCode||'',date:session.date||entry.date||'',time:session.time||entry.sessionTime||'',venue:session.venue||entry.venue||'',
...
function render(claim,live=false){
  ...
  $('#eventName').textContent=claim.eventName||claim.eventCode||'THEARD EVENT';$('#sessionMeta').textContent=[claim.sessionName||claim.sessionCode,claim.sessionCode,claim.date,claim.time].filter(Boolean).join(' / ')||'—';
