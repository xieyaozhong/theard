...
function sessionLabel(session){return [session?.sessionName||session?.sessionCode,session?.sessionCode,session?.date,session?.time].filter(Boolean).join(' / ')||'—'}
...
async function animateResult(result,submittedAttendee=''){
  ...
  const attendeeName=ticket.attendeeName||claim.attendeeName||submittedAttendee;const entry={id:claim.id||`${session.id}:${ticket.id}`,claimId:claim.id||'',claimedAt,recovered:Boolean(claim.recovered),attendeeName,name:pass.name,rarity:pass.rarity,code:ticket.serial,zone,time:new Date(claimedAt).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',hour12:false}),pet,sessionId:session.id,ticketId:ticket.id,serial:ticket.serial,passType:pass.name,verifyToken:ticket.verifyToken,eventName:session.eventName,eventCode:session.eventCode,sessionCode:session.sessionCode,sessionName:session.sessionName,date:session.date,sessionTime:session.time,venue:session.venue,session:{...session},ticket:{...ticket,attendeeName,passType:pass.name,rarity:pass.rarity,zone}};
